...
HashTable.prototype.insert = function(k, v) {
  ...
  if (this._storage.get(index) !== undefined) {
    ...
    for (var i = 0; i < hashArray.length; i++) {
      ...
      if (k === hashArray[i].key) {
        // Replace the existing value at the existing key with
      ...
      // Checks if we have reached the end of the hashArray
        ...
        // This will only happen if a matching key 
        // has not already been found in the hashArray
        hashArray = hashArray.concat(pair);
      }
    }
...
// Function that HashTable instances delegate to. Returns a value based on a provided key
HashTable.prototype.retrieve = function(k) {
  ...
  // Checks if entries exists
    ...
    // Returns the value (which corresponds to the user provided key)
...
HashTable.prototype.remove = function(k) {
  ...
  // Iterates through the hash table's storage
  this._storage.each(function(value, index, storage) {
    ...
    if (Array.isArray(value)) {
      ...
      // Checks if the current index in the iteration is equal to the calculated removeIndex
